...
##############################
def func(x, k=180):
    y = k*x - 2*np.floor( (k*x+1)/2 )
    y = abs(y)*(8*x**8 + 1) / (1 + 10*x**4)
    y += 0.6*np.sin(200*np.pi*x)+0.8*np.cos(160*np.pi*x**2)
    return y


num_epochs = 2000
batch_size = 300
num_training_samples = 60000 # uniform grid samples
num_test_samples = 12345 # random samples
...
for epoch in range(1,1+num_epochs):
    ...
    if epoch % 5 == 0:
        ...
        if epoch % 10 == 0:
            ...
            plt.title(fr'Epoch: {epoch};  $\qquad\quad$    MSE: {e_mse:.2e}',
                      fontsize=16,
                      color="red")
            ax=plt.gca()
            ...
            new_ylim = (current_ylim[0], current_ylim[0]+1.17*e)
            ...
            if not os.path.exists(FPN):
                os.makedirs(FPN)
            plt.savefig(f"{FPN}mmnn1D_epoch{epoch}.png", dpi=50)
            if show_plot:
                plt.show()
