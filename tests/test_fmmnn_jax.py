...
import pytest
from mmnn.fmmnn_jax import FMMNNJax # type: ignore
import optax    
...
def test_fmmnn_init():
    ...
    model = FMMNNJax(ranks=ranks, widths=widths, resnet=False, fix_wb=False)
...
def test_fmmnn_resnet():
    ranks = [2, 4, 4, 1]
    widths = [10, 10, 10]
    model = FMMNNJax(ranks=ranks, widths=widths, resnet=True, fix_wb=False)
...
class FMMNNJaxTrainState(TrainState):
    pass
...
def create_train_state(module, rng, learning_rate, dummy_input):
    ...
    return FMMNNJaxTrainState.create(apply_fn=module.apply, params=params, tx=tx)
